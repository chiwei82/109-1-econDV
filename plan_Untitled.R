# plan_Untitled------------
plan_Untitled=drake::drake_plan(
# > plan begins -----------
# >> downloaddata--------------
downloaddata = {
  library(readr)
...
gg_taiwanEconomicGrowth = {
  dataReady %>%
    ...
    geom_line(
      aes(
        x=å¹<U+FFFD>, y=`ç¶“æ<U+FFFD><U+FFFD>é•·(%)`
      )
    )
},
...
library(drake)
make(plan_Untitled, cache=cacheNew)
}

vis_plan <- function(){
vis_drake_graph(plan_Untitled, cache = cacheNew)
}
